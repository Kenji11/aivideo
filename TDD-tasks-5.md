...
- **Unused Phases**: Multiple phases exist that are no longer used in the pipeline:
  ...
  - `phase4_chunks` - Replaced by `phase3_chunks`
...
2. **Rename Remaining Phases**:
   - Rename `phase3_chunks` → `phase3`
   - Rename `phase4_refine` → `phase4`
...
- ✅ Rename phase3_chunks to phase3
- ✅ Rename phase4_refine to phase4
...
- [x] Search for all references to `phase6` or `phase6_export`:
...
- [x] Search for all references to `phase2_animatic`:
  - Check `backend/app/orchestrator/pipeline.py`
  - Check `backend/app/orchestrator/celery_app.py`
  - Check `backend/app/orchestrator/progress.py`
...
- [x] Search for all references to `phase3_references`:
...
- [ ] Remove from pipeline:
  ...
  - Update chain to go directly from phase2_storyboard to phase3_chunks
...
**Goal:** Delete phase4_chunks directory and all references to it (replaced by phase3_chunks)

- [x] Search for all references to `phase4_chunks` (but NOT `phase3_chunks`):
  - Check `backend/app/orchestrator/pipeline.py`
...
- [x] Verify removal:
  ...
  - Verify pipeline still works (should only use phase3_chunks)

---

### Task 11.5: Rename phase3_chunks to phase3

**Goal:** Rename directory and update all references from `phase3_chunks` to `phase3`

- [x] Rename directory:
  - Rename `backend/app/phases/phase3_chunks/` → `backend/app/phases/phase3/`

- [x] Update all imports:
  - `backend/app/orchestrator/pipeline.py`: 
    - Change: `from app.phases.phase3_chunks.task import generate_chunks as generate_chunks_storyboard`
...
- [x] Update internal imports in phase3 files:
  - Update all `from app.phases.phase3_chunks.*` imports to `from app.phases.phase3.*`
  - Files to check:
    - `phase3/task.py`
    - `phase3/service.py`
...
- [x] Update Celery task names:
  - In `phase3/task.py`: Update `@celery_app.task(bind=True, name="app.phases.phase3_chunks.task.generate_chunks")`
...
- [x] Update celery_app.py:
  - Remove old import/registration for phase3_chunks
...
- [x] Verify rename:
  - Run `grep -r "phase3_chunks" backend/` to ensure no references remain
...
### Task 11.6: Rename phase4_refine to phase4

**Goal:** Rename directory and update all references from `phase4_refine` to `phase4`

- [x] Rename directory:
  - Rename `backend/app/phases/phase4_refine/` → `backend/app/phases/phase4/`

- [x] Update all imports:
  - `backend/app/orchestrator/pipeline.py`: 
    - Change: `from app.phases.phase4_refine.task import refine_video`
    - To: `from app.phases.phase4.task import refine_video`
    - Chain call stays the same: `refine_video.s(user_id)`

- [x] Update internal imports in phase4 files:
  - Update all `from app.phases.phase4_refine.*` imports to `from app.phases.phase4.*`
  - Files to check:
    - `phase4/task.py`
    - `phase4/service.py`
...
- [x] Update Celery task names:
  - In `phase4/task.py`: Update `@celery_app.task(bind=True, name="app.phases.phase4_refine.task.refine_video")`
  - To: `@celery_app.task(bind=True, name="app.phases.phase4.task.refine_video")`

- [x] Update phase names in progress tracking:
  - In `phase4/task.py`: Update all `current_phase="phase4_refine"` → `current_phase="phase4"`
  - In `phase4/task.py`: Update all `phase="phase4_refine"` → `phase="phase4"`
  - In `phase4/task.py`: Update reference to previous phase: `video.current_phase = "phase4_chunks"` → `video.current_phase = "phase3"`

- [x] Update phase output keys:
  - In `phase4/task.py`: Update `video.phase_outputs['phase4_refine']` → `video.phase_outputs['phase4']`
  - In `backend/app/services/status_builder.py`: Update `phase_outputs.get('phase4_refine')` → `phase_outputs.get('phase4')`

- [x] Update celery_app.py:
  - Remove old import/registration for phase4_refine
...
- [x] Verify rename:
  - Run `grep -r "phase4_refine" backend/` to ensure no references remain
...
- [x] Review current chain in `backend/app/orchestrator/pipeline.py`:
  - Current: phase1 → phase2_storyboard → phase3_references → phase3_chunks → phase4_refine
...
- [x] Review `backend/app/orchestrator/progress.py`:
...
- [x] Review `backend/app/services/status_builder.py`:
...
- [x] Update phase output keys:
  ...
  - Update `phase_outputs.get('phase4_refine')` → `phase_outputs.get('phase4')`
  - Update `phase_outputs.get('phase2_animatic')` → Remove fallback (only use phase2_storyboard)

- [x] Update video.phase_outputs references:
...
- [x] Review `backend/app/api/video.py`:
  - Check for phase output key references
...
- [x] Remove test files for deleted phases:
...
- [x] Update test imports:
  ...
  - Update imports from renamed phases:
    - `from app.phases.phase3_chunks.*` → `from app.phases.phase3.*`
    - `from app.phases.phase4_refine.*` → `from app.phases.phase4.*`

- [x] Update test phase names:
...
- [x] Verify directory structure:
  - Check `backend/app/phases/` contains only:
    - `phase1_validate/`
    - `phase2_storyboard/`
    - `phase3/` (renamed from phase3_chunks)
    - `phase4/` (renamed from phase4_refine)

- [x] Verify no old references:
  - Run `grep -r "phase2_animatic" backend/` → Should return nothing
  - Run `grep -r "phase3_references" backend/` → Should return nothing
  - Run `grep -r "phase4_chunks[^_]" backend/` → Should return nothing (only phase3_chunks if rename failed)
  - Run `grep -r "phase4_refine" backend/` → Should return nothing
  - Run `grep -r "phase6" backend/` → Should return nothing

- [x] Verify new phase structure:
  ...
  - Verify phase3 is phase3_chunks renamed
  - Verify phase4 is phase4_refine renamed
