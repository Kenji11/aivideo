    ...
    <div className="max-w-6xl mx-auto animate-fade-in">
      ...
      {filteredVideos.length === 0 ? (
        <div className="card p-16 text-center">
          <p className="text-slate-500 dark:text-slate-400">No videos found matching your criteria</p>
        </div>
      ) : (
            ...
            <div key={video.id} className="card overflow-hidden group hover:shadow-lg transition-all">
              <div className="aspect-video bg-gradient-to-br from-slate-300 to-slate-400 flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 flex items-center justify-center space-x-2">
                  <button className="p-2 bg-white rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-700 transition-colors">
              ...
              <div className="p-4 space-y-3">
                <div>
                  <h3 className="font-semibold text-slate-900 dark:text-slate-100 line-clamp-2">{video.title}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{video.createdAt}</p>
                </div>
                ...
                {video.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {video.tags.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center space-x-1 px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-xs text-slate-600 dark:text-slate-400"
                      >
                    ...
                    {video.tags.length > 3 && (
                      <span className="text-xs text-slate-500 dark:text-slate-400 px-2 py-1">+{video.tags.length - 3}</span>
                    )}
                  </div>
                )}
